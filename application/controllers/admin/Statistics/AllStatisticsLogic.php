 ...
 * @author hwq
 */
class AllStatisticsLogic extends CI_Controller {
	/**
	 * @desc 构造函数
	 */
	public function __construct(){
		parent::__construct();
		$this->load->model('Statistics/Allstatistics_model', 'allstatistics');
		checksession();
	}
	...
	public function indexPage(){
		$this->load->view('admin/Statistics/AllStatisticsList');
	}
	...
	public function onLoadMsg_WXMenuStatistics(){
		//查询数据库
		$data = $this->allstatistics->selectMsg_WXMenuStatistics();
		//var_dump($data);exit;
		//
		//菜单类型
		/*foreach ($data as $k => $v) {
	...
	public function onLoadMsg_HistoryStatistics(){
		...
		//查询数据库,获得按日期反序排序的数组
		$data = $this->allstatistics->selectMsg_HistoryStatistics($startTime,$endTime);
		//var_dump($data);exit;
		//反转数组,使得按照日期升序排序
