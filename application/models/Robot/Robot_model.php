...
class Robot_model extends CI_Model{
	...
	public function insertProblem($title,$questiontype,$answer,$keyword,$creator,$created,$modifyer,$modified){
		...
		$res = $this->mysqlhelper->Insert('gst_hunan_dkf.robot_question',$insertArr);
		...
		if ($res){
		 	...
		 	if(!empty($keyword)){
				...
				$res2 = $this->mysqlhelper->Insert('gst_hunan_dkf.robot_keyword',$insertArr2);
		 		}
	...
	public function updateProblem($questionid,$title,$questiontype,$answer,$keyword,$modifyer,$modified){
		...
		if ($num > 0 ){
		 	...
		 	$sql = "delete from gst_hunan_dkf.robot_keyword where questionid = ".$questionid;
		 	...
		 	if($res2 > 0)
		 	{
		 		if(!empty($keyword)){
					...
					$res3 = $this->mysqlhelper->Insert('gst_hunan_dkf.robot_keyword',$insertArr2);
			 		}
	...
	public function selectProblem($status,$keyword,$pageOnload){
		...
		$sql = "select questionid,title,questiontype,answer,group_concat(b.shortname SEPARATOR '|') f,(
		select group_concat(keyword SEPARATOR '|') k from gst_hunan_dkf.robot_keyword	 WHERE questionid = a.questionid ) keyword from	gst_hunan_dkf.robot_question a left JOIN gde_roadold b on locate(concat(',',b.roadoldid,','),concat(',',a.answer,','))>0 where 1 = 1 ";
	...
	public function selectOneProblem($questionid){
		$sql = "select questionid,title,questiontype,answer,(select group_concat(keyword separator '|') k from gst_hunan_dkf.robot_keyword WHERE questionid=a.questionid) keyword from gst_hunan_dkf.robot_question a where a.questionid = ".$questionid;
	...
	public function deleteProblem($questionid){
		...
		if($res > 0){
			$sql = "delete from gst_hunan_dkf.robot_question where questionid = ".$questionid;
			$res = $this->db->query($sql);
			if($res > 0){
				return true;
			}else{
	...
	public function getRoadoldData(){
		$sql="select roadoldid,concat( newcode, '-' , shortname) as shortname
			FROM
				gde_roadold
			where 
				1=1";
	...
	public function selectExcelProblem($typeSel,$keyword){
