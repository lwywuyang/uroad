...
class AdPic_model extends CI_Model{
	...
	function getAdPicData($StartTime,$EndTime,$stateSel,$pageOnload){
		$sql = "select * FROM gde_ad WHERE 1 = 1";
		$params = array();
	...
	public function saveAdPic($AdPicdata){
		...
		if ($AdPicdata['id'] == '0') {
			$sql = 'insert into gde_ad (imageurl,redirecturl,state,seq,created,modified)values(?,?,?,?,now(),now())';
			$params = array($AdPicdata['imageurl'],$AdPicdata['redirecturl'],0,$AdPicdata['seq']);
            $content = '新增app首页广告 '.$imghtml;
            saveLog($content,2010004);
			$this->db->query($sql,$params);
			if ($this->db->affected_rows() <= 0) {
				return '新增失败!';
			}
		}else{
			$sql = 'update gde_ad set imageurl=?,redirecturl=?,seq=?,modified=now() where id=?';
			$params = array($AdPicdata['imageurl'],$AdPicdata['redirecturl'],$AdPicdata['seq'],$AdPicdata['id']);
            $content = '修改app首页广告 '.$imghtml;
            saveLog($content,2010004);
			$this->db->query($sql,$params);
			if ($this->db->affected_rows() < 0) {
				return '更新失败!';
	...
	public function updateState($id,$state){
        ...
        $content = '修改app首页广告 状态为'.$statusname;
        saveLog($content,2010004);
		$sql = 'update gde_ad set state=?,modified=now() where id=?';
