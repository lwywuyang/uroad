...
<head>
    ...
    <script type="text/javascript" src="https://webapi.amap.com/maps?v=1.3&key=aaaacfd2066bd9ff25658d55741539ae&plugin=AMap.Geocoder"></script>
    ...
    <div class="tab-pane active" id="nav_tfsjxq" >
                ...
                <tr>
                    ...
                    <td style="border-top:2px solid #0F70D8;" rowspan="4">
                        <div class="operator_div">
                                <div class="operatorstatus"></div>
                                <div>处理人：<span id="operatorname"></span></div>
                                <div class="operator_btn">
                                    <input id="handle" style="display:none" type="button" value="处理" class="btn btn-primary" onclick="savehandle(1002001)" />
        ...
        <div style="margin-bottom: 30px;height: 400px;">
            <div class="row" style="margin-left:0">
                <div class="col-md-6 col-sm-6 col-xs-6" style="border: 1px solid #ddd;height: 400px;">
                    ...
                    <div style="height: 400px;margin-bottom: 20px;">
                        ...
                        <div>
                            <div class="col-md-12 processcontenttitle">
                                <div class="col-md-5 col-sm-5 jztitle" style="border-right: 1px solid #eee;">路段名称</div>
                                <div class="col-md-1 col-sm-1 jztitle" style="border-right: 1px solid #eee;">时速</div>
                                <div class="col-md-2 col-sm-2 jztitle" style="border-right: 1px solid #eee;">拥堵距离</div>
                                <div class="col-md-4 col-sm-4 jztitle">更新时间</div>
                            </div>
                            <div id="processcontent" style="clear:both;height: 315px;overflow: hidden;">
            ...
            <div>
                ...
                <div>
                    <div class="col-md-12 handlecontenttitle">
                        <div class="col-md-2 col-sm-2 jztitle" style="border-right: 1px solid #eee;">操作人</div>
                        <div class="col-md-4 col-sm-4 jztitle" style="border-right: 1px solid #eee;">事件状态</div>
                        <div class="col-md-2 col-sm-2 jztitle" style="border-right: 1px solid #eee;">操作状态</div>
                        <div class="col-md-4 col-sm-4 jztitle">操作事件</div>
                    </div> 
                    <div id="handlecontent" style="clear:both;height: 150px;overflow: hidden;">
        ...
        <div style="margin-bottom: 30px;">
            <div class="gaodetitle" style="border-bottom: 2px solid #0F70D8;height: 45px;margin-bottom: 10px">
...
<!--             <div class="title-font" style="border-bottom: 3px solid #0beaea;height: 45px;margin-bottom: 10px">
            ...
            <table class="table mb30 table-bordered " id="sjglsave">
                <tr id="quanxiansss">
                    <td colspan="4" class="form-inline" id="cfyxdwddsbbz" width="150px" >
                        <input type="radio" id="cfyxdwddsbbz_input" val="yxdwsb"  name="sjglradio" />触发有效但未达到上报标准
                    </td>
                </tr>

                <tr>
                    <td class="form-inline" id="sjeventid" width="150px" style="line-height: 40px;">
                ...
                <tr class="qxclass">
                    <td style="text-align:center">
                        <input type="submit" name="add" value="上传图片"  id="upload1" class="btn btn-success qxclass"
                               onclick="uploadFile();"/>
...
<script>
    ...
    var nowpage = "<?php echo empty($nowpage)?1:$nowpage?>";//当前页码
    var type = "<?php echo $type?>";//区分按钮类型 1 查看 0 处理
    var isnew = 0;//是否为已经处理状态 0 是未处理 1是已处理

    var empidcs = "<?php echo empty($empidcs)?'':$empidcs?>";
    // console.log(empidcs);
    ...
    function goaddsj(){
        var src = "<?php echo base_url('index.php/admin/MsgPublish/RoadEventLogic/indexPage?eventtype=1006001') ?>&gaodecztype=1"+"&ids="+gaodeeventid;
        $(window.top.document).find('#iframeContent').eq(0).attr('src','').attr('src',src);
    }


    function fh(){
        var src = "<?php echo base_url('index.php/admin/GaoDeLIst/GaoDeListLogic/gaoDeIndexPage') ?>?selecttype="+selecttype+"&nowpage="+nowpage;
        $(window.top.document).find('#iframeContent').eq(0).attr('src','').attr('src',src);
    }

    $(function(){
        // if(empidcs=='NS20160927085016255178'){
        //     $("#quanxiansss").show();
        // }
            ...
            if(data.Success){
                ...
                for (var i = 0; i < trafficprocesslength; i++) {
                    // if(i<5){
                        ...
                        trafficprocesstr+='<div class="col-md-5 col-sm-5"  style="border-right: 1px solid #eee;">'+data.data[i].roadname+"</div>";
                        trafficprocesstr+='<div class="col-md-1 col-sm-1"  style="border-right: 1px solid #eee;">'+data.data[i].jamspeed+"</div>";
                        trafficprocesstr+='<div class="col-md-2 col-sm-2"  style="border-right: 1px solid #eee;">'+data.data[i].jamdist+"</div>";
                        trafficprocesstr+='<div class="col-md-4 col-sm-4" >'+data.data[i].inserttime+"</div>";
        ...
        JAjax('admin/GaoDeLIst/GaoDeListLogic','onLoadHandleProcess',{gaodeeventid:gaodeeventid},function (data){
            if(data.Success){
                ...
                for (var i = 0; i < handleprocesslength; i++) {
                    ...
                    var operatorname = data.data[i].operatorname==null?'系统提醒':data.data[i].operatorname;
                     handleprocesstr+='<div class="col-md-12 handlecontenttitle" style="border-top:0px solid">';
                        handleprocesstr+='<div class="col-md-2 col-sm-2" style="border-right: 1px solid #eee;">'+operatorname+"</div>";
                        handleprocesstr+='<div class="col-md-4 col-sm-4" style="border-right: 1px solid #eee;">'+eventstatusname+"</div>";
                        handleprocesstr+='<div class="col-md-2 col-sm-2" style="border-right: 1px solid #eee;">'+data.data[i].operatorstatusname+"</div>";
                        handleprocesstr+='<div class="col-md-4 col-sm-4">'+data.data[i].operatortime+"</div>";
                        handleprocesstr+="</div>";
                    // if(i<5){
                        // var eventstatusname = data.data[i].eventstatusname==null?'未处理':data.data[i].eventstatusname;
...
// console.log(gaodeeventid);
                if(data.data.eventstatus==1001001){//未处理
                    ...
                    for (var i = 0; i < data.data.relationeventall.length; i++) {
                        ...
                        eventallhtml+='<td colspan="4" class="form-inline" style="width:350px;" id="sjeventid">';
                        eventallhtml+='<input  type="radio" num="'+data.data.relationeventall[i]['eventno']+'" val="'+data.data.relationeventall[i]['eventid']+'" class="sjglradio" name="sjglradio">&nbsp;<span class="eventspan" id="eventno'+data.data.relationeventall[i]['eventid']+'" >'+data.data.relationeventall[i]['reportout']+'</span></td>';
                    ...
                    if(data.data.eventstatus==1001002){//有效
                        var eventallhtml = "";
                            eventallhtml+= "<tr class='eventalltr'>";
                            eventallhtml+= '<td colspan="4"><span onclick="lookevent(\''+data.data.gsteventid+'\')" style="cursor: pointer;color:#2dd462">'+data.data.reportout+'</span></td>';
                    ...
                    }else if(data.data.eventstatus==1001006){
                        // console.log(123);
                        $("#sjglsave").hide();
                        $("#cfyxdwdscl").show();
                        $("#jtkfyjbutton").hide();
                    ...
                    $(".operatorstatus").text(data.data.eventstatusname);
                }
    ...
    function lookevent(eventid){
        var src = "<?php echo base_url('index.php/admin/MsgPublish/RoadEventLogic/indexPage?eventtype=1006001') ?>&gaodecztype=2"+"&ids="+eventid;
        $(window.top.document).find('#iframeContent').eq(0).attr('src','').attr('src',src);
    }
    ...
    function saveHandleResult(e){
        ...
        if(eventid=='yxdwsb'){//等于触发有效但未达到上报标准
            eventstatus = 1001006;
        }

        if(eventid!='invalid' && eventid!=undefined && eventid!='yxdwsb'){
    ...
    function readFile(obj){
        ...
        reader.onload = function(e){
            ...
            JAjax('admin/GaoDeLIst/GaoDeListLogic','imgupload2',{imagebase64:strArry[1]},function (data){
                if(data.Success)
                {
    ...
    function savehandle(status){
        JAjax('admin/GaoDeLIst/GaoDeListLogic','savehandle',{gaodeeventid:gaodeeventid,status:status,selecttype:selecttype},function (data){
            if(data.Success){
                if(status==1002001){
                    $("#handle").hide();
                    $("#cancelhandle").show();
                    ...
                    $("#cancelhandle").hide();
                    $(".qxclass").hide();
                    $("#operatorname").text("暂无");
                    $(".operatorstatus").text("待处理");
                    ShowMsg("已取消处理！");
                }
                getHandleProcess();
            }else{
    ...
    function generatePoi(){
        var startstationmap = new AMap.Marker({
            map: map,
            position: [startstationx, startstationy],
            icon: new AMap.Icon({
        ...
        new AMap.Marker({
            map: map,
            position: [endstationx, endstationy],
            icon: new AMap.Icon({
                size: new AMap.Size(40, 50),  //图标大小
     ...
     * 插入站名
     * @Author   RaK
     * @DateTime 2017-06-07T09:57:01+0800
     * @return   {[type]}                 [description]
     */
    function insertRoadPoiName(roadPoiName,startstationmap){
        var info = [];
        var infoWindow;
        info.push("<b>" + roadName + "</b>");
        infoWindow = new AMap.InfoWindow({
            content: info.join("<br/>")  //使用默认信息窗体框样式，显示信息内容
        });
        infoWindow.open(map, startstationmap.getPosition());
    }
    ...
    function matchingPoi(){
        JAjax('admin/GaoDeLIst/GaoDeListLogic','matchingPoi',{gaodeeventid:gaodeeventid,xys:detailData,roadName:roadName},function (data){
            if(data.status=="OK"){
                ...
                $("#startmile").text("K"+startStation.replace('.','+'));
                $("#endmile").text("K"+endStation.replace('.','+'));
            }else{
                ShowMsg("匹配失败");
            }
        },null,false);
