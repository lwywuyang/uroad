...
<!-- 离线地图 -->
  <head>
    ...
    <script>
        ...
        function myGraphicsClickHandler(evt) {
         ...
         function save() {
            var x1=graphicPoint.geometry.x;
            var y1=graphicPoint.geometry.y;
            // alert(y1);
            x1= parseFloat(x1);
            y1= parseFloat(y1);
            x1=x1.toFixed(6);
            y1=y1.toFixed(6);
            // xubox_iframe2 
            // $(this).contents().find("#xubox_iframe1").getxy(x1,y1);
            // $(window.top.document).children("iframeid")[2].getxy(x1,y1);
            // $(window.top.document).children("iframe")[1].layer.close(index);
            // alert(iframeid);
            // top.frames["#"+iframeid].getxy(x1,y1);
            // // $("#"+iframeid).window.getxy(x1,y1);
            parent.getxy(x1,y1);
  ...
  <body  >
        ...
        <div class="footer">
            ...
            <input type="button"  id='save' value="确定" class="btn btn-primary" onclick="save();" />
            <input type="button" id='closeLayer' value="取消" class="btn btn-primary" onclick="closeLayer();" />
